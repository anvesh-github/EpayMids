...
public class XlsReader {
		...
		try {
			...
			if (cell.getCellType() == HSSFCell.CELL_TYPE_STRING)
				...
				if (HSSFDateUtil.isCellDateFormatted(cell)) {
					...
					String dataFromExcel = String.valueOf(cell
							.getNumericCellValue());
					Double value = Double.parseDouble(dataFromExcel);
					Long longValue = value.longValue();
					dataFromExcel = new String(longValue.toString());
					cellText = dataFromExcel;
	...
	public boolean setCellData(String sheetName, String colName, int rowNum,
		...
		try {
			...
			if (data.equals("PASSED")) {
				...
				cs.setAlignment(CellStyle.ALIGN_CENTER);
				cell.setCellStyle(cs);
			} else if (data.equals("SKIPPED") | data.equals("NOTRUN")) {
				CellStyle cs = workbook.createCellStyle();
				cs.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
	...
	public Hashtable getRowData(String sheetName, int rowNum) {
		...
		for (int i = 0; i < row.getLastCellNum(); i++) {
			// System.out.println(row.getCell(i).getStringCellValue().trim());
			headers.add(row.getCell(i).getStringCellValue().trim());
		...
		for (int i = 0; i < row.getLastCellNum(); i++) {
			...
			if (cell.getCellType() == HSSFCell.CELL_TYPE_STRING)
				...
				cellsData.add(String.valueOf(cell.getBooleanCellValue()));
		...
		for (int i = 0; i < cellsData.size(); i++) {
			...
			rowData.put(headers.get(i), cellsData.get(i));
		}
